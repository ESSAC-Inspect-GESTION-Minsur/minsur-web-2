...
interface AssignVehicleTypeProps {
  isOpen: boolean
  templateVehicleTypes: VehicleType[]
  update: (vehicleTypes: VehicleType[]) => void
  onClose: () => void
}

const AssignVehicleType = ({ isOpen, templateVehicleTypes, update, onClose }: AssignVehicleTypeProps): ReactElement => {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>): void => {
    ...
    void templatesService.assignVehicleType(templateId, vehicleTypeId)
      .then(response => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} className='min-w-[300px] sm:min-w-[600px]'>
      <div className='p-6'>
        <h2 className='uppercase font-bold text-center text-lg'>Asignar tipo de vehículo al checklist {selectedTemplate?.name}</h2>
        <Divider></Divider>
        {
